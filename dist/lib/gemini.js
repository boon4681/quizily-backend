...
const retriable = (m) => /503|429|overloaded|rate|quota/i.test(m);
async function generateWithRetry(genAI, models, prompt) {
    let last;
    for (const model of models) {
        for (let i = 0; i < 2; i++) {
            try {
                ...
                last = e;
                if (!retriable(String(e?.message || '')) || i === 1)
                    break;
                await sleep(700 * (i + 1));
            }
        }
...
function chunk(text, size = 8000, overlap = 500) {
    if (text.length <= size)
        return [text];
    const out = [];
    let p = 0;
...
async function summarizeIfLarge(genAI, models, text) {
    const MAX_DIRECT = 20000;
    if (text.length <= MAX_DIRECT)
        return text;
    const bullets = [];
...
function buildQuestionTypeInstructions(questionType) {
...
function normalizeQuestion(question, forcedType) {
    const base = question.type === 'TRUE_FALSE'
...
function normalizeMultipleChoiceQuestion(question) {
...
export async function generateQuizFromText(params) {
    const { text, title, description, questionCount = 5, difficulty = 'BEGINNER', modelId, questionType, } = params;
