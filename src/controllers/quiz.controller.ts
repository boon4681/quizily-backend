...
export const quizController = {
  ...
  async generate(c: any) {
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        const qCount = form.get('questionCount') as string | null
  ...
  async getById(c: any) {
    try {
      ...
      if (!result) return c.json({ message: 'Quiz not found' }, 404)
      return c.json(result)
    } catch (err) {
      console.error(err)
      return c.json({ message: (err as any)?.message || 'Failed to fetch quiz' }, 500)
    }
  },
